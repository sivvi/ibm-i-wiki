...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| ACQ (E) | device-name | workstn-file | _ | ER |

### 行为说明

ACQ 操作获取由 device-name 指定的程序设备，并将其附加到由 workstn-file 指定的 WORKSTN 文件。如果设备可用，ACQ 将其附加到文件；如果不可用或已附加到文件，则发生错误。

ACQ 不会执行任何输入或输出操作。可用于多设备文件或单设备文件的错误恢复。

处理 ACQ 异常（文件状态码大于 1000）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| REL (E) | program-device | file-name | _ | ER |

### 行为说明

REL 操作从指定的 WORKSTN 文件释放由 program-device 指定的程序设备。

当没有程序设备获取到 WORKSTN 文件时，下一次 READ-by-file-name 或 cycle-read 会获得 EOF 条件。REL 操作可用于多设备文件或单设备文件的错误恢复。

**注意**：释放记录锁请使用 UNLOCK 操作。

处理 REL 异常（文件状态码大于 1000）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| COMMIT (E) | boundary | _ | _ | ER |

### 行为说明

COMMIT 操作使自上一次 COMMIT 或 ROLBK 操作以来（或自承诺控制操作开始以来）对已打开进行承诺控制的文件所做的所有更改永久化。boundary 操作数指定承诺控制边界（*ALL、*RECLK 或省略）。

发出 COMMIT 操作的程序不需要在承诺控制下有任何文件。COMMIT 操作不会改变文件位置。

**注意**：必须在文件描述规范上指定 **COMMIT** 关键字才能打开文件进行承诺控制。同时需要在 IBM i 上使用 CL 命令 **STRCMTCTL** 启动承诺控制。

处理 COMMIT 异常（文件状态码大于 1000）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
ROLBK 操作执行以下操作：
- 消除自上一次 COMMIT 或 ROLBK 操作以来对文件所做的所有更改
- 释放所有在承诺控制下的文件的记录锁
- 将文件重新定位到上一次 COMMIT 操作时的位置（或文件 OPEN 时的位置，如果之前没有 COMMIT 操作）

发出 ROLBK 操作的程序不需要在承诺控制下有任何文件。回滚会影响激活组或作业中所有在承诺控制下的文件，包括其他程序所做的更改。

处理 ROLBK 异常（程序状态码 802-805）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| EXCEPT | except-name | _ | _ | _ |

### 行为说明

EXCEPT 操作允许在详细计算或总计算期间写入一条或多条记录。

- **带 except-name**：只检查并写入与指定 EXCEPT 名称相同的异常记录（前提是条件指示器满足）
- **不带 except-name**：检查并写入输出规范中位置 30-39 没有 EXCEPT 名称的所有异常记录

异常记录在输出规范上通过在位置 17 中指定 **E** 来标识。仅异常记录可以包含 EXCEPT 名称。

如果异常记录由输出规范上的溢出指示器条件化，则该记录仅在 RPG IV 周期的溢出部分或获取溢出期间写入，不在 EXCEPT 操作处理时写入。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| FEOD (EN) | file-name | _ | _ | ER |

### 行为说明

FEOD 操作向主文件、副文件或全过程文件发出逻辑数据结束信号。与 CLOSE 操作的区别：程序不会与设备或文件断开连接，文件可以在后续文件操作中再次使用，而无需再次指定显式 OPEN 操作。

**N 扩展符**：可指定用于对使用阻塞的输出型 DISK 或 SEQ 文件执行 FEOD。如果指定了 N 扩展符，块中的任何未写记录将被写出到数据库，但不一定会写出到非易失性存储。使用 N 扩展符可以提高性能。

处理 FEOD 异常（文件状态码大于 1000）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| FORCE | file-name | _ | _ | _ |

### 行为说明

FORCE 操作允许选择要从中读取下一条记录的文件。只能用于主文件或副文件。

如果在程序周期中处理了多个 FORCE 操作，则除最后一个外其他所有都被忽略。FORCE 必须在详细时间发出，不能在总时间发出。

FORCE 操作覆盖程序通常选择记录的多文件处理方法。但是，要处理的第一条记录始终通过正常方法选择。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| NEXT (E) | program-device | file-name | _ | ER |

### 行为说明

NEXT 操作强制多设备文件的下一个输入来自由 program-device 操作数指定的程序设备，前提是输入操作是周期读取或 READ-by-file-name。任何读取操作（包括 CHAIN、EXFMT、READ 和 READC）都会结束上一个 NEXT 操作的效果。如果在输入操作之间多次指定 NEXT，则只处理最后一个。

NEXT 操作码只能用于多设备文件。

处理 NEXT 异常（文件状态码大于 1000）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| POST (E) | program-device | file-name INFDS name | _ | ER |

### 行为说明

POST 操作将信息放入 INFDS（文件信息数据结构）。这些信息包括：
- 特定于 RPG I/O 的文件反馈信息
- 文件的打开反馈信息
- 文件的输入/输出反馈信息和设备相关反馈信息 **或** 获取属性信息

**program-device 操作数**：指定程序设备名称以获取该特定设备的信息。如果指定了程序设备，文件必须定义为 WORKSTN 文件。

**file-name 操作数**：指定文件名，与文件关联的 INFDS 中将登记该文件的信息。

**行为注意**：即使未处理 POST 操作码，它的存在也会影响 RPG IV 语言的运行方式。POST 操作的存在会影响一个或多个文件的反馈登记方式。

处理 POST 异常（文件状态码大于 1000）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
...
| 代码 | Factor 1 | Factor 2 | 结果字段 | 指示器 |
|------|----------|----------|---------|--------|
| UNLOCK (E) | name (file or data area) | _ | _ | ER |

### 行为说明

UNLOCK 操作用于解锁数据区和释放记录锁。

**解锁数据区**：
- name 操作数必须是数据区的名称，或保留字 **\*DTAARA**
- 指定 `*DTAARA` 时，程序中所有已锁定的数据区都被解锁
- 数据区必须在 *DTAARA DEFINE 语句的结果字段中已指定，或在定义规范上带有 DTAARA 关键字

**释放记录锁**：
- name 操作数必须是更新磁盘文件的名称
- UNLOCK 释放最近一次锁定的记录

**位置 71-72、75-76 必须空白**。

处理 UNLOCK 异常（程序状态码 401-421、431、432）：指定操作码扩展符 'E' 或错误指示器 ER，不能同时指定两者。
